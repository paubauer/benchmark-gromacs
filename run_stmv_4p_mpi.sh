...
mpirun -x HIP_VISIBLE_DEVICES=0,1,2,3 --mca pml ucx -x UCX_MM_SEG_SIZE=60k --mca coll_ucc_enable 1 --mca coll_ucc_priority 100 -x UCX_ROCM_COPY_H2D_THRESH=256 -np 4 ./set_mpi_affinities.sh $ROOT/Gromacs-mpi/build-mpi/bin/gmx_mpi mdrun -resethway -nsteps -1 -maxh 0.1 -v -stepout 1000 -noconfout -nstlist 300 -nb gpu -bonded gpu -pme gpu -update gpu -pin off -ntomp 24 -npme 1 -g md_1gpu.log -gpu_id $GPUID -s stmv/topol.tpr -tunepme no
